...
def trigger_camera_action(position):
    ...
    lat, lon = position
    url = "http://dev-server/device-manager/api/v1/devices/onvifcam-dev-1/send-command"
    payload = {
        "command": "look_at",
        "parameters": {
            "latitude": lat,
    ...
    try:
        ...
        if response.status_code == 200:
            log(f"✅ Camera pointed successfully to {position}")
        else:
            log(f"❌ Camera command failed with status {response.status_code}: {response.text}")
    except Exception as e:
        log(f"⚠️ Camera command error: {e}")


def on_connect(client, userdata, flags, rc):
    log(f"Connected with result code {rc}")
    # client.subscribe("/device/vorpal/vorpal-service/platform/map/events")
    client.subscribe("/device/vorpal/HoffmansPC/platform/map/events")
